  ...
  try {
    ...
    planitByReference[group.normalised_reference] = {
      ...
      records: records.map((record) => ({
        uid: record.uid ?? null, reference: record.reference ?? null, name: record.name ?? null,
...
const planitHealth = sourceHealth({ attempted, succeeded: completed, statusCode: lastStatus, priorGoodAt: state.last_good_at, message: healthMessage });
...
await atomicJson(snapshotPath, { schema: "pipelinenews.official-source-snapshot.v2", generated_at: now, spine: { projects: projects.length, reference_groups: groups.length }, source_health: { planit: planitHealth, govuk: govukHealth }, planit_by_reference: planitByReference, govuk_items: govukItems, google_news: { enabled: true, credibility_score: 30, role: "noisy discovery only" } });
await atomicJson(statePath, { schema: "pipelinenews.official-source-cursor.v2", next_index: nextIndex, total_groups: groups.length, last_run_at: now, last_good_at: now });
console.log(`official-source poll: ${projects.length} projects; PlanIt ${completed}/${attempted}; GOV.UK ${govukSucceeded}/${govukAttempted}; next=${nextIndex}`);
