...
export function signalForProjectNewsV1(project) {
...
export function drawNewsNewsV1Base() {
...
async function loadNews() {
  for (const [label, url] of NEWS_SOURCES) {
    try {
      ...
      console.error(`News V1 source unavailable: ${label}`, error);
    }
  }
  document.getElementById("stories").innerHTML = '<div class="news-empty">News V1 newspaper unavailable. REPD analytics below remain live.</div>';
  document.getElementById("newsMeta").textContent = "newspaper unavailable";
  return null;
}

export function loadNewsNewsV1Base() {
  if (!newsLoadPromise) {
...
export function bindNewspaperNewsV1Base(onNewsLoaded) {
