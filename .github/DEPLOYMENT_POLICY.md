...
`.github/workflows/authorise-v8-fast-pages.yml` is the bounded owner-authorisation path for generation `202608270055`. It may add only the immutable authorisation record in a one-parent commit after every candidate and browser gate passes. It may then request the separate Pages `workflow_dispatch` run with that exact authorisation commit as the required `expected_sha`; it may not assemble, upload or deploy a Pages artifact itself. Pages must fail if the dispatch commit, the `expected_sha`, the authorisation commit or remote `main` differ.

For this promotion, `releases/`, `data/` and `archive/` are frozen at the green candidate-output commit. Live verification must hash all seven candidate outputs, all sixteen pinned detail partitions, pinned Chart.js and the unchanged stable pointer before browser-proofing the direct candidate route. `releases/current.json` remains unchanged.
