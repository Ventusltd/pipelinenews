...
async function exerciseNewsAndCharts(page) {
  ...
  assert.match(initial.meta, /45 UK.*19 international.*133 headlines/i, "news metadata parity changed");
  assert.match(initial.more, /133 MATCHES/i, "all 133 headlines are not reachable through progressive news loading");
  ...
  assert.equal(articleIds.size, 133, "not every compact headline is reachable through bounded news pagination");
...
async function main() {
  ...
  try {
    ...
    process.stdout.write(`${JSON.stringify({
      schema: "pipelinenews.v8.fast-browser-proof.v1",
