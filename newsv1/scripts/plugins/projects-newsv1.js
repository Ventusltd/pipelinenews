...
import { loadCanonicalProjectsNewsV1Release } from "../data/canonical-projects-newsv1-release.js";
import {
  formatCapacityV9_2,
  formatLargestV9_2,
...
import { signalForProjectNewsV1 } from "./newspaper-newsv1-base.js";
...
function renderTable() {
  ...
  body.innerHTML = pageRows.map((project) => {
    ...
    const signal = signalForProjectNewsV1(project);
...
function renderPager() {
  const previous = document.getElementById("projectPrev");
  const next = document.getElementById("projectNext");
  const statusElement = document.getElementById("projectPageStatus");
  if (!previous || !next || !statusElement) return;
  const pageCount = Math.max(1, Math.ceil(filtered.length / ROWS_PER_PAGE));
  const start = filtered.length ? pageIndex * ROWS_PER_PAGE + 1 : 0;
  const end = Math.min((pageIndex + 1) * ROWS_PER_PAGE, filtered.length);
  previous.disabled = pageIndex === 0;
  next.disabled = pageIndex >= pageCount - 1 || filtered.length === 0;
  statusElement.textContent = filtered.length
    ? `SHOWING ${start.toLocaleString("en-GB")}–${end.toLocaleString("en-GB")} OF ${filtered.length.toLocaleString("en-GB")} · PAGE ${(pageIndex + 1).toLocaleString("en-GB")} OF ${pageCount.toLocaleString("en-GB")}`
    : "NO PROJECTS MATCH THIS FILTER";
}

function setProjectControlsReady(ready) {
  dataReady = ready;
  document.querySelectorAll("#tech button, #status button, #county, #sortProjects, #search, #clearFilters, #exportInline, #sortUpdated")
    .forEach((control) => { control.disabled = !ready; });
  const exportLink = document.getElementById("export");
  exportLink.setAttribute("aria-disabled", String(!ready));
  document.querySelector(".tablewrap").setAttribute("aria-busy", String(!ready));
}

function renderFirstPartitionPreview({ projects, loaded_count: loadedCount, total_count: totalCount }) {
  if (dataReady) return;
  filtered = [...projects];
  state.filtered = filtered;
  pageIndex = 0;
  renderTable();
  const results = document.getElementById("resultsMeta");
  results.textContent = `${loadedCount.toLocaleString("en-GB")} verified records ready · loading full ${totalCount.toLocaleString("en-GB")} record index…`;
  results.dataset.hydration = "partial";
  document.getElementById("releaseMeta").textContent = "News V1 performance interface · first verified partition visible · completing canonical hydration";
}
...
function apply({ syncUrl = true, resetPage = true } = {}) {
...
function downloadCsv(event) {
  ...
  anchor.href = url;
  anchor.download = `pipelinenews_uk_renewables_newsv1_${new Date().toISOString().slice(0, 10)}.csv`;
  anchor.click();
...
export async function loadProjectsNewsV1() {
  try {
    const model = await loadCanonicalProjectsNewsV1Release({ onFirstPartition: renderFirstPartitionPreview });
    all = [...model.projects];
    filtered = all;
    ...
    apply({ syncUrl: false });
    setProjectControlsReady(true);
    document.getElementById("resultsMeta").dataset.hydration = "complete";
    document.getElementById("releaseMeta").textContent = `News V1 performance interface · V${release.data_parent.release} canonical data spine · all ${all.length.toLocaleString("en-GB")} qualifying records loaded · 100-row pages`;
  } catch (error) {
    console.error(error);
    document.getElementById("tbody").innerHTML = '<tr><td colspan="11" style="text-align:center;color:#ff6666">Canonical Q2 REPD data unavailable. News V1 has failed closed.</td></tr>';
    document.getElementById("resultsMeta").textContent = "canonical data unavailable";
  }
}

export function refreshProjectsNewsV1() {
  if (metadata) renderTable();
}

export function bindProjectControlsNewsV1() {
  if (controlsBound) return;
  controlsBound = true;
  setProjectControlsReady(false);
  ...
  document.getElementById("export").onclick = (event) => { if (dataReady) downloadCsv(event); else event.preventDefault(); };
  document.getElementById("exportInline").onclick = (event) => { if (dataReady) downloadCsv(event); };
  ...
  document.getElementById("projectNext").onclick = () => {
    if ((pageIndex + 1) * ROWS_PER_PAGE >= filtered.length) return;
    pageIndex += 1;
    renderTable();
  };
