...
import { intelligenceForArticleNewsV7 } from "./intelligence-newsv7.js";

const NEWS_SOURCES = Object.freeze([
  ["PipelineNews release", "dist/major_project_news_v9_5_1.json"],
  ["Frozen GlobalGrid source", "https://raw.githubusercontent.com/Ventusltd/globalgrid2050/824a23cd0cf9f90a9df942f1b37a09c2dc6472b7/dist/major_project_news_v9_5_1.json"],
]);
...
export function signalForProjectNewsV7(project) {
  const item = newsIndex.get(String(project.repd_ref));
  if (!item) return { label: "—", cls: "none", note: "no exact canonical PRIMARY_MATCH" };
  const intelligence = intelligenceForArticleNewsV7(item.article_id);
  return {
    ...signalLabel(item),
    note: intelligence
      ? `canonical PRIMARY_MATCH ${Number(item.confidence || 0)}% · event ${String(intelligence.event_type).toLowerCase()} remains headline-derived · roles abstained · research held`
      : `canonical PRIMARY_MATCH ${Number(item.confidence || 0)}% · unverified event · ${item.published || "date unavailable"}`,
  };
}

function intelligenceMarkup(item) {
  if (item.canonical_relevant !== true) return "";
  const intelligence = intelligenceForArticleNewsV7(item.article_id);
  if (!intelligence) return '<span class="story-intelligence"><span class="intelligence-badge">CUMULATIVE EVIDENCE LOADING</span></span>';
  const operators = intelligence.operator_labels.map((row) => row.source_label).join(" / ");
  return `<span class="story-intelligence"><span class="intelligence-badge">${escapeHtml(intelligence.event_type)} · UNVERIFIED</span><span class="intelligence-badge evidence">REPD OPERATOR: ${escapeHtml(operators)}</span><span class="intelligence-badge">ROLES ABSTAINED</span><span class="intelligence-badge">RESEARCH HELD</span></span>`;
}
...
export function drawNewsNewsV7Base() {
...
async function fetchPayload(label, url) {
  ...
  try {
    const response = await fetch(target, { cache: "default", signal: controller.signal });
...
async function loadNews() {
  for (const [label, url] of NEWS_SOURCES) {
    try {
      const result = await fetchPayload(label, url);
      renderNews(result.data, result.label);
      return result.data;
    } catch (error) {
      console.error(`News V7 baseline source unavailable: ${label}`, error);
    }
  }
  document.getElementById("stories").innerHTML = '<div class="news-empty">News V7 newspaper unavailable. REPD analytics below remain live.</div>';
  document.getElementById("newsMeta").textContent = "newspaper unavailable";
  return null;
}

export function loadNewsNewsV7Base() {
  if (!newsLoadPromise) {
    newsLoadPromise = loadNews().catch((error) => {
...
export function bindNewspaperNewsV7Base(onNewsLoaded) {
