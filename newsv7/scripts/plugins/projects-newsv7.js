...
import { loadCanonicalProjectsNewsV7Release } from "../data/canonical-projects-newsv7-release.js";
import {
  formatCapacityV9_2,
  formatLargestV9_2,
...
import { signalForProjectNewsV7 } from "./newspaper-newsv7-base.js";
...
function renderTable() {
  ...
  body.innerHTML = pageRows.map((project) => {
    ...
    const signal = signalForProjectNewsV7(project);
...
function renderFirstPartitionPreview({ projects, loaded_count: loadedCount, total_count: totalCount }) {
  ...
  document.getElementById("releaseMeta").textContent = "News V7 cumulative interface · first verified partition visible · completing canonical hydration";
}
...
function downloadCsv(event) {
  ...
  anchor.href = url;
  anchor.download = `pipelinenews_uk_renewables_newsv7_${new Date().toISOString().slice(0, 10)}.csv`;
  anchor.click();
...
export async function loadProjectsNewsV7() {
  try {
    const model = await loadCanonicalProjectsNewsV7Release({ onFirstPartition: renderFirstPartitionPreview });
    all = [...model.projects];
    filtered = all;
    ...
    document.getElementById("releaseMeta").textContent = `News V7 cumulative interface · V${release.frozen_source.release} baseline · all ${all.length.toLocaleString("en-GB")} qualifying records loaded · 100-row pages`;
  } catch (error) {
    console.error(error);
    document.getElementById("tbody").innerHTML = '<tr><td colspan="11" style="text-align:center;color:#ff6666">Canonical Q2 REPD data unavailable. News V7 has failed closed.</td></tr>';
    document.getElementById("resultsMeta").textContent = "canonical data unavailable";
  }
}

export function refreshProjectsNewsV7() {
  if (metadata) renderTable();
}

export function bindProjectControlsNewsV7() {
