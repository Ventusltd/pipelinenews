...
async function main() {
  ...
  const cartridgePath = path.join(releaseA, `assets/${GENERATION}-atlas-v9-deep-link.mjs`);
  ...
  for (const project of projects) {
    ...
    const valid = project.geometry_status === "valid"
      && url.origin === "https://ventusltd.github.io"
      && url.pathname === "/gridatlas/202608291239-atlas-v9/"
  ...
  const sections = {
    ...
    atlas_links: {
      ...
      identity_only_query: classified(
        cartridge.buildAtlasV9DeepLink(eastPye) === `${ATLAS_BASE}?repd_ref=17494`,
        cartridge.buildAtlasV9DeepLink(eastPye),
        { expectedChange: true },
      ),
      identity_only_query_beacon_fen: classified(
        cartridge.buildAtlasV9DeepLink(beaconFen) === `${ATLAS_BASE}?repd_ref=13599`,
        cartridge.buildAtlasV9DeepLink(beaconFen),
        { expectedChange: true },
      ),
    ...
    real_public_receiver: {
      ...
      inbound_exact_project: browserProof === null ? item("UNKNOWN", "browser proof missing") : classified(browserProof.inbound_repd_ref === "17494" && browserProof.filtered_projects === 1 && browserProof.project_row_id === "repd-17494", browserProof.inbound),
      selected_receiver: browserProof === null ? item("UNKNOWN", "browser proof missing") : classified(browserProof.external_atlas_network_used === true && browserProof.atlas_repd_ref === "17494" && String(browserProof.receiver_text).includes("REPD 17494 selected"), { url: browserProof.receiver_url, text: browserProof.receiver_text }),
      beacon_fen_receiver: browserProof === null ? item("UNKNOWN", "browser proof missing") : classified(
        browserProof.beacon_fen?.repd_ref === "13599"
          && browserProof.beacon_fen?.filtered_projects === 1
          && browserProof.beacon_fen?.project_row_id === "repd-13599"
          && browserProof.beacon_fen?.outbound_href === `${ATLAS_BASE}?repd_ref=13599`
          && String(browserProof.beacon_fen?.receiver_text).includes("REPD 13599 selected"),
        browserProof.beacon_fen,
      ),
    },
  };
  ...
  const report = {
    ...
    classification: failed === 0 ? "VERIFIED_PIPELINENEWS_ATLAS_V9_SUCCESSOR" : "REJECTED_PIPELINENEWS_ATLAS_V9_SUCCESSOR",
