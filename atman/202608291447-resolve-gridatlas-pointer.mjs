...
function parseArgs(argv) {
  ...
  assert.match(String(args["expected-pointer-sha256"] || ""), /^[0-9a-f]{64}$/u);
  assert.match(String(args["expected-commit"] || ""), /^[0-9a-f]{40}$/u);
  assert.match(String(args["contractual-golden-repd-ref"] || ""), /^\d+$/u);
...
async function githubJson(url, token) {
  const response = await fetch(url, {
    headers: {
      ...
      "User-Agent": "PipelineNews-GridAtlas-Pointer-Resolver/2",
...
export function validatePointerBundle(bundle, expectations) {
  ...
  assert.equal(pointerRecord.sha256, expectations.pointerSha256, "GridAtlas pointer digest changed");
  assert.equal(bundle.commit, expectations.commit, "GridAtlas pointer commit changed");
  ...
  assert.equal(current.query_contract?.identity_rule, "EXACT_REPD_REF_ONLY");
  ...
  assert.equal(manifest.truth_contract?.routing_identity_is_exact_repd_ref, true);
  const sentinels = manifest.route_contract?.browser_sentinels;
  assert.ok(Array.isArray(sentinels) && sentinels.length > 0, "GridAtlas route sentinels missing");
  assert.equal(sentinels.every((value) => /^\d+$/u.test(String(value))), true);
  ...
  return Object.freeze({
    ...
    authentication: Object.freeze({
      ...
      contractual_probe_source: "PIPELINENEWS_SOURCE_CONTRACT",
      receiver_ui_parity: "NOT_ASSERTED",
    }),
    receiver: Object.freeze({
      ...
      identity_rule: "EXACT_REPD_REF_ONLY",
      golden_repd_ref: expectations.contractualGoldenRepdRef,
      browser_sentinels: sentinels.map(String),
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const receipt = validatePointerBundle(bundle, {
    ...
    contractualGoldenRepdRef: args["contractual-golden-repd-ref"],
  });
