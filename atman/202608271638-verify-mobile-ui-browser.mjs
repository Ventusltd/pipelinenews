...
import CONTRACT, { GENERATION } from "./202608271638-mobile-ui-invariants.mjs";
...
async function runRotateCell(browser, target) {
  ...
  try {
    ...
    const snapshots = [await nodeSnapshot(page, cdp, "initial-P1", initialViewport)];
    for (let cycle = 1; cycle <= rotate.cycles; cycle += 1) {
      ...
      if (cycle === 1) {
        screenshots.landscape = await captureAndRestoreRotationScreenshot({
          capture: () => captureScreenshots(page, target.id, rotate.id, "landscape"),
          restoreViewport: () => applyRotationViewport(page, cdp, landscape, rotate.dpr),
          restoreSafeArea: () => applySafeArea(cdp, "L1", landscape.orientation),
          settle: () => page.waitForTimeout(rotate.settle_ms),
        });
      }
      snapshots.push(await nodeSnapshot(page, cdp, `cycle-${cycle}-L1`, landscape));
      ...
      await page.waitForTimeout(rotate.settle_ms);
      snapshots.push(await nodeSnapshot(page, cdp, `cycle-${cycle}-P1`, portrait));
    }
