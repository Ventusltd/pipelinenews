...
async function observe(browser, label, url, viewport) {
  ...
  if (label === "candidate") {
    ...
    const startupRelationship = requests.filter((path) => path.includes(`${CANDIDATE_GENERATION}-federated-relationships`) || path.includes(`${CANDIDATE_GENERATION}-relationship-governance-status`));
  ...
  if (label === "candidate") {
    ...
    await page.locator("#federatedRelationshipOpen").click();
    const lazy = {
      module: requests.filter((path) => path.endsWith(`/${CANDIDATE_GENERATION}-federated-relationships.js`)).length,
      payload: requests.filter((path) => path.endsWith(`/${CANDIDATE_GENERATION}-relationship-governance-status.json`)).length,
...
const result = {
  schema: "pipelinenews.v9-6-2-complete-equivalence-report.v1",
  generation: GENERATION,
  candidate_generation: CANDIDATE_GENERATION,
  source_commit: process.env.GITHUB_SHA,
  workflow_run_id: process.env.GITHUB_RUN_ID,
  playwright_version: "1.55.0",
  predecessor_audit: { generation: "202608282348", run_id: 33218336294, disposition: "GREEN_INCOMPLETE_NOT_PROMOTION_ELIGIBLE" },
  ...
  state: { candidate: "ARTIFACT_ONLY", current_pointer_changed: false, pages_changed: false, catalogue_changed: false },
};
