...
def validate_release(root: Path, requested: str, *, replay_sources: bool = True) -> dict:
    ...
    manifest_commit = git_text(root, "log", "-1", "--format=%H", "--", manifest_path.relative_to(root).as_posix())
...
def validate_timestamp_folder_release(root: Path, release_id: str) -> dict:
    ...
    if release_manifest.get("schema") == "pipelinenews.current-atlas-link-release.v2":
