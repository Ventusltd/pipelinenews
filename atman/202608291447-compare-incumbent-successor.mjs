...
function validPipelineGoldenEvidence(proof, repdRef) {
  const golden = proof?.contractual_golden;
  const pipeline = golden?.pipeline_evidence;
  if (typeof golden?.pipeline_present !== "boolean") return false;
  if (!pipeline || pipeline.repd_ref !== repdRef || pipeline.present !== golden.pipeline_present) return false;
  if (!pipeline.present) {
    return pipeline.tested === false
      && pipeline.pipeline_filtered_count === 0
      && Array.isArray(pipeline.pipeline_visible_repd_refs)
      && pipeline.pipeline_visible_repd_refs.length === 0;
  }
  let inbound;
  try {
    inbound = new URL(pipeline.inbound_url);
  } catch {
    return false;
  }
  return pipeline.tested === true
    && pipeline.pipeline_filtered_count === 1
    && JSON.stringify(pipeline.pipeline_visible_repd_refs) === JSON.stringify([repdRef])
    && inbound.searchParams.get("repd_ref") === repdRef
    && pipeline.outbound_href === `${ATLAS_BASE}?repd_ref=${repdRef}`
    && pipeline.receiver_url === `${ATLAS_BASE}?repd_ref=${repdRef}`
    && pipeline.receiver_evidence?.cards?.some((text) => text.includes(`REPD ${repdRef}`));
}
...
async function main() {
  ...
  const sections = {
    ...
    atlas_links: {
      exact_inbound_match_contract: classified(
        releaseManifest.atlas_v9_deep_link?.inbound_match_semantics === "EXACT_PROJECT_REPD_REF"
          && releaseManifest.atlas_v9_deep_link?.general_search_parameter === "q"
          && releaseManifest.atlas_v9_deep_link?.relationship_context_allowed_for_q === true
          && releaseManifest.atlas_v9_deep_link?.relationship_context_allowed_for_repd_ref === false
          && registry.cache_contract?.atlas_deep_link?.inbound_match_semantics === "EXACT_PROJECT_REPD_REF"
          && cartridge.ATLAS_V9_DEEP_LINK_CONTRACT.inbound_match_semantics === "EXACT_PROJECT_REPD_REF",
        {
          release: releaseManifest.atlas_v9_deep_link,
          registry: registry.cache_contract?.atlas_deep_link,
          cartridge: cartridge.ATLAS_V9_DEEP_LINK_CONTRACT,
        },
        { expectedChange: true },
      ),
    ...
    bidirectional_routing: {
      inbound_alias: classified(
        runtime.includes('let requestedRepdRef = "";')
          && runtime.includes('requestedRepdRef = /^\\d+$/u.test(repdRefParameter) ? repdRefParameter : "";')
          && runtime.includes('if (requestedRepdRef && String(item.repd_ref) !== requestedRepdRef) continue;')
          && runtime.includes('url.searchParams.set("repd_ref", requestedRepdRef)')
    ...
    real_public_receiver: {
      ...
      contractual_receiver_golden: browserProof === null ? item("UNKNOWN", "browser proof missing") : classified(
        browserProof.contractual_golden?.repd_ref === contractualGoldenRef
          ...
          && validPipelineGoldenEvidence(browserProof, contractualGoldenRef)
      ...
      optional_sentinels_when_present: browserProof === null ? item("UNKNOWN", "browser proof missing") : classified(
        [
          ...
          return entry
            ...
            && (!project ? (
              entry.tested === false
              && entry.pipeline_filtered_count === 0
              && Array.isArray(entry.pipeline_visible_repd_refs)
              && entry.pipeline_visible_repd_refs.length === 0
            ) : (
              entry.tested === true
              && entry.pipeline_filtered_count === 1
              && JSON.stringify(entry.pipeline_visible_repd_refs) === JSON.stringify([repdRef])
              && (repdRef !== "13599" || entry.related_repd_13600_excluded === true)
