...
import CONTRACT, { GENERATION } from "./202608271656-mobile-ui-invariants.mjs";
...
const FULL_PAGE_CAPTURE_LIMITS = Object.freeze({
  max_dimension_css_px: 16_384,
  max_area_css_pixels: 4_194_304,
  max_project_rows: 1_000,
});
...
if (
  CONTRACT.project_posture?.owner !== "Ventus Ltd"
  || CONTRACT.project_posture?.application !== "non-commercial-open-source"
  || CONTRACT.project_posture?.publisher_redistribution_rights !== "source-specific-not-inferred"
) {
  throw new Error("Mobile UI contract project posture is absent or invalid");
}
...
function serialiseError(error) {
  if (!(error instanceof Error)) return { name: "Error", message: String(error), stack: null };
  const serialised = { name: error.name, message: error.message, stack: error.stack || null };
  for (const key of [
    "rotation_phase",
    "rotation_operation",
    "expected_geometry",
    ...
    if (error[key] !== undefined) serialised[key] = error[key];
  }
  if (error.cause instanceof Error) {
    serialised.cause = serialiseError(error.cause);
  }
  return serialised;
}
...
function selectFullPageCapture(dimensions, limits) {
  const documentWidth = dimensions.document_width;
  const documentHeight = dimensions.document_height;
  const documentPixels = documentWidth * documentHeight;
  const reasons = [];
  if (
    ...
    reasons.push(`invalid document dimensions ${documentWidth}x${documentHeight}`);
  } else if (
    documentWidth > limits.max_dimension_css_px
    || documentHeight > limits.max_dimension_css_px
  ) {
    reasons.push(
      `document ${documentWidth}x${documentHeight} exceeds ${limits.max_dimension_css_px} CSS px side limit`,
    );
  }
  if (!Number.isSafeInteger(documentPixels) || documentPixels <= 0) {
    reasons.push(`invalid or unsafe document area ${documentPixels}`);
  } else if (documentPixels > limits.max_area_css_pixels) {
    reasons.push(
      `document ${documentWidth}x${documentHeight} area ${documentPixels} exceeds `
      + `${limits.max_area_css_pixels} CSS pixels`,
    );
  }
  if (dimensions.project_row_count > limits.max_project_rows) {
    reasons.push(`project row count ${dimensions.project_row_count} exceeds ${limits.max_project_rows}`);
  }
  return {
    complete_document: reasons.length === 0,
    reason: reasons.length > 0 ? `bounded fallback: ${reasons.join("; ")}` : null,
    document_css_pixels: documentPixels,
    limits,
  };
}
...
function pngDimensions(buffer) {
  ...
  if (
    ...
    throw new Error("Screenshot raster is not a bounded PNG with an IHDR header");
  }
  return {
...
function assertViewportScreenshotPixels(fileName, observed, expected) {
  if (observed.width === expected.width && observed.height === expected.height) return true;
  const error = new Error(
    `Viewport screenshot ${fileName} is ${observed.width}x${observed.height}; `
    + `expected ${expected.width}x${expected.height}`,
  );
  error.name = "ScreenshotGeometryError";
  error.expected_geometry = expected;
  error.observed_geometry = observed;
  throw error;
}

async function captureScreenshots(page, targetId, cellId, suffix = "", expectedViewport = null) {
  ...
  const dimensions = await page.evaluate(() => ({
    ...
    project_row_count: document.querySelectorAll("#tbody > tr").length,
  }));
  const expectedViewportPixels = {
    width: expectedViewport?.width ?? dimensions.viewport_width,
    height: expectedViewport?.height ?? dimensions.viewport_height,
  };
  const boundedJpeg = async (filePath, fullPage) => {
    ...
    const rasterPixels = pngDimensions(raster);
    if (!fullPage) {
      assertViewportScreenshotPixels(path.basename(filePath), rasterPixels, expectedViewportPixels);
    }
    let buffer;
    let quality;
    ...
    return {
      ...
      pixel_width: rasterPixels.width,
      pixel_height: rasterPixels.height,
      expected_viewport_pixels: fullPage ? null : expectedViewportPixels,
      pixel_dimensions_match_expected: fullPage ? null : true,
  ...
  const fullPagePolicy = selectFullPageCapture(dimensions, FULL_PAGE_CAPTURE_LIMITS);
  let full;
  if (fullPagePolicy.complete_document) {
  ...
  return {
    ...
    full: {
      ...full,
      complete_document: fullPagePolicy.complete_document,
      note: fullPagePolicy.reason,
      capture_policy: fullPagePolicy,
    },
    dimensions,
  };
}
...
async function nodeSnapshot(page, cdp, phase, expectedViewport) {
  ...
  const expectedOrientationMatches = expectedViewport.orientation === "landscape"
    ? pageState.landscape
    : pageState.portrait;
  if (
    pageState.inner_width !== expectedViewport.width
    || pageState.inner_height !== expectedViewport.height
    || !expectedOrientationMatches
  ) {
    throw new Error(
      `Rotation snapshot ${phase} layout viewport does not match `
      + `${expectedViewport.width}x${expectedViewport.height} ${expectedViewport.orientation}`,
    );
  }
  return {
    phase,
    ...
    layout_viewport_matches_expected: true,
...
async function observeRotationGeometry(page) {
  ...
  try {
    const browser = await page.evaluate(() => ({
      ...
      visual_viewport_width: visualViewport?.width ?? null,
      visual_viewport_height: visualViewport?.height ?? null,
      visual_viewport_scale: visualViewport?.scale ?? null,
      screen_width: screen.width,
      screen_height: screen.height,
    ...
    return {
      ...
      playwright_viewport: playwrightViewport,
      browser,
    ...
    return {
      ...
      playwright_viewport: playwrightViewport,
      browser: null,
      observation_error: serialiseError(error),
    };
  }
}

function appendRotationTrace(trace, entry) {
  if (trace.length >= 96) trace.shift();
  trace.push(entry);
}

function rotationGeometryMatchesExpected(observed, expected) {
  if (!expected) return true;
  const browser = observed?.browser;
  const playwright = observed?.playwright_viewport;
  if (!browser || !playwright || observed.observation_error) return false;
  const orientationMatches = expected.orientation === "landscape"
    ? browser.landscape && !browser.portrait
    : browser.portrait && !browser.landscape;
  return (
    playwright.width === expected.width
    && playwright.height === expected.height
    && browser.inner_width === expected.width
    && browser.inner_height === expected.height
    && browser.device_pixel_ratio === expected.device_pixel_ratio
    && orientationMatches
  );
}

async function runRotationPhase(page, trace, descriptor, operation) {
  const before = await observeRotationGeometry(page);
  appendRotationTrace(trace, {
    phase: descriptor.phase,
    operation: descriptor.operation,
    status: "STARTED",
    expected_geometry: descriptor.expected_geometry || null,
    observed_geometry: before,
  });
  try {
    const value = await operation();
    const after = await observeRotationGeometry(page);
    if (!rotationGeometryMatchesExpected(after, descriptor.expected_geometry)) {
      const mismatch = new Error(
        `Post-operation geometry does not match phase ${descriptor.phase}`,
      );
      mismatch.name = "RotationGeometryMismatchError";
      mismatch.expected_geometry = descriptor.expected_geometry || null;
      mismatch.observed_geometry = after;
      throw mismatch;
    }
    appendRotationTrace(trace, {
      phase: descriptor.phase,
      ...
      status: "COMPLETED",
      expected_geometry: descriptor.expected_geometry || null,
      observed_geometry: after,
    });
    return value;
  } catch (cause) {
    const observed = await observeRotationGeometry(page);
    appendRotationTrace(trace, {
      phase: descriptor.phase,
      operation: descriptor.operation,
      status: "FAILED",
      ...
      cause: serialiseError(cause),
    });
    const browserGeometry = observed.browser;
    const observedLabel = browserGeometry
      ? `${browserGeometry.inner_width}x${browserGeometry.inner_height}; `
        + `portrait=${browserGeometry.portrait}; landscape=${browserGeometry.landscape}`
      : "unavailable";
    const error = new Error(
      `Rotation phase ${descriptor.phase} (${descriptor.operation}) failed; observed ${observedLabel}`,
      { cause },
    );
    error.name = "RotationPhaseError";
    error.rotation_phase = descriptor.phase;
    error.rotation_operation = descriptor.operation;
    error.expected_geometry = descriptor.expected_geometry || null;
    error.observed_geometry = observed;
    error.rotation_trace = trace.slice();
    throw error;
  }
}

function rotationViewportReady({ width, height, orientation }) {
  return (
    innerWidth === width
    && innerHeight === height
...
async function applyRotationViewport(page, trace, viewport, dpr, phase) {
  const expectedGeometry = {
    width: viewport.width,
    height: viewport.height,
    orientation: viewport.orientation,
    device_pixel_ratio: dpr,
    driver: "playwright-page-set-viewport-size",
  };
  return runRotationPhase(page, trace, {
    phase,
    operation: "page.setViewportSize",
    expected_geometry: expectedGeometry,
  }, async () => {
    await page.setViewportSize({ width: viewport.width, height: viewport.height });
    await page.waitForFunction(rotationViewportReady, viewport, { timeout: 5_000 });
    await page.evaluate(() => new Promise((resolve) => {
      requestAnimationFrame(() => requestAnimationFrame(resolve));
    }));
  });
...
function rotationMeasurement(snapshots) {
  ...
  return {
    viewport_driver: "playwright-page-set-viewport-size",
    viewport_readiness_gate: "inner-viewport-and-match-media-orientation",
    screen_fields_observed_not_gated: true,
    snapshots,
...
async function runStaticCell(browser, target, cell) {
  ...
  try {
    ...
    const screenshots = await captureScreenshots(page, target.id, cell.id, "", cell);
...
async function captureAndRestoreRotationScreenshot(steps) {
  const screenshot = await steps.capture();
  await steps.restoreViewport();
  await steps.restoreSafeArea();
  await steps.settle();
  return screenshot;
}
...
async function runRotateCell(browser, target) {
  ...
  const capturedAt = new Date().toISOString();
  const rotationTrace = [];
  const expectedGeometry = (viewport) => ({
    width: viewport.width,
    height: viewport.height,
    orientation: viewport.orientation,
    device_pixel_ratio: rotate.dpr,
  });
  const phase = (phaseId, operation, viewport, action) => runRotationPhase(page, rotationTrace, {
    phase: phaseId,
    operation,
    expected_geometry: viewport ? expectedGeometry(viewport) : null,
  }, action);
  try {
    const ready = await phase(
      "initial-P1-navigation",
      "navigate-and-ready",
      initialViewport,
      () => navigate(page, target),
    );
    await applyRotationViewport(
      page,
      rotationTrace,
      initialViewport,
      rotate.dpr,
    ...
    const initialSafeArea = await phase(
      "initial-P1-safe-area",
      "applySafeArea",
      initialViewport,
      () => applySafeArea(cdp, "P1", initialViewport.orientation),
    );
    const screenshots = {
      initial: await phase(
        "initial-P1-screenshot",
        "captureScreenshots",
        initialViewport,
        () => captureScreenshots(page, target.id, rotate.id, "initial", initialViewport),
      ),
      landscape: null,
      final: null,
    };
    const snapshots = [await phase(
      "initial-P1-node-snapshot",
      "nodeSnapshot",
      initialViewport,
      () => nodeSnapshot(page, cdp, "initial-P1", initialViewport),
    )];
    for (let cycle = 1; cycle <= rotate.cycles; cycle += 1) {
      ...
      await applyRotationViewport(
        ...
        landscape,
        rotate.dpr,
        `cycle-${cycle}-L1-transition`,
      );
      await phase(
        `cycle-${cycle}-L1-safe-area`,
        "applySafeArea",
        landscape,
        () => applySafeArea(cdp, "L1", landscape.orientation),
      );
      await phase(
        `cycle-${cycle}-L1-settle`,
        "waitForTimeout",
        landscape,
        () => page.waitForTimeout(rotate.settle_ms),
      );
      if (cycle === 1) {
        screenshots.landscape = await captureAndRestoreRotationScreenshot({
          capture: () => phase(
            `cycle-${cycle}-L1-screenshot`,
            "captureScreenshots",
            landscape,
            () => captureScreenshots(page, target.id, rotate.id, "landscape", landscape),
          ),
          restoreViewport: () => applyRotationViewport(
            page,
            rotationTrace,
            landscape,
            rotate.dpr,
            `cycle-${cycle}-L1-post-screenshot-restore`,
          ),
          restoreSafeArea: () => phase(
            `cycle-${cycle}-L1-post-screenshot-safe-area`,
            "applySafeArea",
            landscape,
            () => applySafeArea(cdp, "L1", landscape.orientation),
          ),
          settle: () => phase(
            `cycle-${cycle}-L1-post-screenshot-settle`,
            "waitForTimeout",
            landscape,
            () => page.waitForTimeout(rotate.settle_ms),
          ),
      ...
      snapshots.push(await phase(
        `cycle-${cycle}-L1-node-snapshot`,
        "nodeSnapshot",
        landscape,
        () => nodeSnapshot(page, cdp, `cycle-${cycle}-L1`, landscape),
      ));
      ...
      await applyRotationViewport(
        ...
        portrait,
        rotate.dpr,
        `cycle-${cycle}-P1-transition`,
      );
      await phase(
        `cycle-${cycle}-P1-safe-area`,
        "applySafeArea",
        portrait,
        () => applySafeArea(cdp, "P1", portrait.orientation),
      );
      await phase(
        `cycle-${cycle}-P1-settle`,
        "waitForTimeout",
        portrait,
        () => page.waitForTimeout(rotate.settle_ms),
      );
      snapshots.push(await phase(
        `cycle-${cycle}-P1-node-snapshot`,
        "nodeSnapshot",
        portrait,
        () => nodeSnapshot(page, cdp, `cycle-${cycle}-P1`, portrait),
      ));
    }
    const finalViewport = rotate.sequence[2];
    screenshots.final = await phase(
      "final-P1-screenshot",
      "captureScreenshots",
      finalViewport,
      () => captureScreenshots(page, target.id, rotate.id, "final", finalViewport),
    );
    ...
    return {
      ...
      captured_at: capturedAt,
      ready,
      diagnostics: { ...diagnostics, rotation_trace: rotationTrace },
    ...
    error.rotation_trace ||= rotationTrace.slice();
    error.browser_diagnostics = diagnostics;
    throw error;
  } finally {
    await context.close();
  }
...
function unavailableRecord(target, cell, error, rotate = false) {
  const failure = serialiseError(error);
  const browserDiagnostics = error?.browser_diagnostics || {
    console_errors: [],
    page_errors: [],
    failed_requests: [],
    http_errors: [],
  ...
  return {
    ...
    diagnostics: {
      ...browserDiagnostics,
      failure_class: rotate ? "rotation-instrument" : "navigation-readiness-or-instrument",
      navigation_error: failure,
      rotation_failure: rotate ? failure : null,
    },
