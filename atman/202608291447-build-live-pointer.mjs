...
async function main() {
  ...
  assert.equal(receiver.inbound_match_semantics, "EXACT_PROJECT_REPD_REF");
  assert.equal(receiver.general_search_parameter, "q");
  assert.equal(receiver.relationship_context_allowed_for_q, true);
  assert.equal(receiver.relationship_context_allowed_for_repd_ref, false);
  ...
  assert.equal(typeof proof.contractual_golden?.pipeline_present, "boolean");
  const pipelineGolden = proof.contractual_golden?.pipeline_evidence;
  assert.equal(pipelineGolden?.repd_ref, receiver.golden_repd_ref);
  assert.equal(pipelineGolden?.present, proof.contractual_golden.pipeline_present);
  if (pipelineGolden.present) {
    assert.equal(pipelineGolden.tested, true);
    assert.equal(pipelineGolden.pipeline_filtered_count, 1);
    assert.deepEqual(pipelineGolden.pipeline_visible_repd_refs, [receiver.golden_repd_ref]);
    assert.equal(new URL(pipelineGolden.inbound_url).searchParams.get("repd_ref"), receiver.golden_repd_ref);
    assert.equal(pipelineGolden.outbound_href, `${atlasBase.href}?repd_ref=${receiver.golden_repd_ref}`);
    assert.equal(pipelineGolden.receiver_url, `${atlasBase.href}?repd_ref=${receiver.golden_repd_ref}`);
    assert.ok(pipelineGolden.receiver_evidence?.cards?.some((text) => text.includes(`REPD ${receiver.golden_repd_ref}`)), "PipelineNews contractual-golden receiver card missing");
  } else {
    assert.equal(pipelineGolden.tested, false);
    assert.equal(pipelineGolden.pipeline_filtered_count, 0);
    assert.deepEqual(pipelineGolden.pipeline_visible_repd_refs, []);
  }
  ...
  for (const optional of proof.optional_sentinels || []) {
    ...
    if (optional.present) {
      ...
      assert.equal(optional.pipeline_filtered_count, 1);
      assert.deepEqual(optional.pipeline_visible_repd_refs, [optional.repd_ref]);
      if (optional.repd_ref === "13599") assert.equal(optional.related_repd_13600_excluded, true);
      ...
      assert.equal(optional.tested, false);
      assert.equal(optional.pipeline_filtered_count, 0);
      assert.deepEqual(optional.pipeline_visible_repd_refs, []);
    }
  }
