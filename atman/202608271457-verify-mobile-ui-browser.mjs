...
import CONTRACT, { GENERATION } from "./202608271457-mobile-ui-invariants.mjs";
...
const CELL_RECORD_LIMIT_BYTES = 64 * 1024;
...
const candidateGeneration = process.env.MOBILE_UI_CANDIDATE_GENERATION || "202608271329";
...
async function waitUntilReady(page, target) {
  ...
  if (target.ready === "fast-1329") {
    ...
    if (boot.failed === "true" || boot.ready !== "true") {
  ...
  return page.evaluate(async ({ contract, cellSpec, safeAreaState, menuExerciseState }) => {
    ...
    const safeAreaSnapshot = (label) => {
      ...
      for (const element of document.querySelectorAll(selectors.interactives)) {
        ...
        if (!inViewport(rect)) continue;
        const edges = [];
        if (insets.top > 0 && rect.top < insets.top && rect.bottom > 0) edges.push("top");
        if (insets.right > 0 && rect.right > innerWidth - insets.right && rect.left < innerWidth) edges.push("right");
        if (insets.bottom > 0 && rect.bottom > innerHeight - insets.bottom && rect.top < innerHeight) edges.push("bottom");
        if (insets.left > 0 && rect.left < insets.left && rect.right > 0) edges.push("left");
        if (!edges.length) continue;
        count += 1;
        if (intersections.length < 20) {
          intersections.push({ element: elementLabel(element), edges, rect: rectData(rect) });
        }
      }
    ...
    const copyFonts = bodyCopies.map((element) => ({
      element: elementLabel(element),
    ...
    const minimumCopyFont = copyFonts.length
      ? Math.min(...copyFonts.map(({ font_size_px: size }) => size))
      : null;
    const i11 = {
...
try {
  ...
  for (const targetId of CONTRACT.rotate_cell.targets) {
...
const run = {
  ...
  records: recordFiles,
  expected_record_count: CONTRACT.cells.length * targets.length + CONTRACT.rotate_cell.targets.length,
