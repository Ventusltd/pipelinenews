...
async function proveProject(page, context, candidate, base, repdRef, required) {
  ...
  if (!present) {
    ...
    return { repd_ref: repdRef, present: false, tested: false };
  }
...
async function main() {
  ...
  try {
    ...
    // PipelineNews project snapshot does not contain that newer REPD identity.
    const pipelineGolden = await proveProject(page, context, candidate, base, receiver.golden_repd_ref, false);
    assert.equal(pipelineGolden.present, false, "inherited PipelineNews snapshot unexpectedly contains GridAtlas contractual golden");
