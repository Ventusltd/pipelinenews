...
export function widerMetricActions(row, index, label = 'GRID') {
  ...
  return refs.map(ref => {
    ...
    return `<button type="button" class="action-metric" style="cursor:pointer;min-height:44px;font:inherit;color:inherit" data-repd-metric="${esc(ref)}" title="REPD ${esc(ref)}: ${esc(explanation)}">${label} ${refs.length > 1 ? esc(ref) + ' ' : ''}${valid ? `<b>${hit.k.toFixed(2)}</b><span class="unit">km</span>` : 'unavailable'}</button>`;
  }).join(' ');
}
