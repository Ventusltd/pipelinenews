...
export async function mountWiderFleet({ host, payloadAsset, presentSummary, onSpineRepaint, metricActions = () => "" }) {
  ...
  const visible = () => filterWiderRows(active ? rows.filter((row) => row.rt === active) : [],localQuery);

  function clearWider() {
    localFilter.disabled=true;localQuery='';localFilter.value='';
    const cleanUrl=new URL(location.href);cleanUrl.searchParams.delete('wider_q');history.replaceState(null,'',cleanUrl);
    select.value = "";
