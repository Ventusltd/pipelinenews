...
function render(host, product) {
  ...
  panel.innerHTML = `
    ...
    <div class="gbe-cards">
      <div class="gbe-card"><b>£${number(price.decade_mean, 2)}</b><span>mean of complete daily means · £/MWh</span></div>
      ...
      <div class="gbe-card"><b>£${number(low.value, 2)}</b><span>lowest settlement period · ${escapeHtml(low.date)}</span></div>
      <div class="gbe-card"><b>£${number(high.value, 2)}</b><span>highest settlement period · ${escapeHtml(high.date)}</span></div>
    </div>
    ...
    <p class="gbe-note"><strong>Historic context only.</strong> ${escapeHtml(product.not_a_forecast)}
