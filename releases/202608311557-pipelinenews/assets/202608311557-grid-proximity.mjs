 ...
 * Generation 202608311557. DRAFT - deployment: not-authorised.
 *
...
const BAND_RANK = { STRONG: 0, MODERATE: 1, DISTANT: 2, REMOTE: 3, UNKNOWN: 4 };
...
export function mountGridProximity({ host, payloadAsset }) {
  ...
  function renderSort(d) {
    ...
    const value = (r, k) => (
      k === "_gp" ? BAND_RANK[r.grid_probable.band]
    ...
    function draw() {
      ...
      wrap.querySelectorAll("button[data-why]").forEach((btn) => {
        btn.addEventListener("click", () => {
          ...
          box.innerHTML = `<div class="gp-readout"><h4>${esc(r.grid_probable.band)} · ${esc(r.name)}</h4>
  ...
  function acquire(row, canvas, readout) {
    ...
    const step = (now) => {
      ...
      if (readout) {
        readout.innerHTML = `
          <h4>${t >= 1 ? "Target acquired" : "Acquiring…"}</h4>
          <dl>
            ...
            <dt>GRID</dt><dd>${esc((row.grid_probable && row.grid_probable.band || "").toLowerCase())}</dd>
          </dl>
      ...
      if (t < 1) scopeFrame = requestAnimationFrame(step);
    };
    scopeFrame = requestAnimationFrame(step);
  }
  ...
  function renderTarget(d, preselect) {
    ...
    const wrap = document.createElement("div");
    wrap.innerHTML = `
      <p class="section-title">DRAW STRAIGHT TO NEAREST SUBSTATION</p>
  ...
  markTabs();
  note.textContent = "WAIT · five tabs · choose one to request the derived proximity index";
