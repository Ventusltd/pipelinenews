...
const EXPECTED_COMPILER_METHOD = "pipelinenews-v8-fast-dictionary-index-lazy-detail-v1";
const EXPECTED_CACHE_IDENTITY = "8ab1807551bc77066e46e01cb0690dfaa41d473d8433644046f56b6984579b06";
const REGISTRY_URL = `data/${GENERATION}-v8-fast-registry.json`;
...
async function ensureSearchSupplement() {
  ...
  invariant(payload.generation === GENERATION, "search generation mismatch");
  invariant(payload.cache_identity === EXPECTED_CACHE_IDENTITY, "search cache identity mismatch");
...
function newsMatches(row) {
  ...
  if (newsQuery) {
    const haystack = normalise([row[NEWS_FIELD.headline], row[NEWS_FIELD.project], row[NEWS_FIELD.operator], row[NEWS_FIELD.county], row[NEWS_FIELD.source], row[NEWS_FIELD.event], row[NEWS_FIELD.repdRef], row[NEWS_FIELD.projectId]].join(" "));
...
function newsHtml(row) {
  ...
  const quality = row[NEWS_FIELD.canonical] === true
...
async function loadNews() {
  ...
  try {
    ...
    invariant(payload.generation === GENERATION, "news generation mismatch");
    invariant(payload.cache_identity === EXPECTED_CACHE_IDENTITY, "news cache identity mismatch");
...
async function boot() {
  ...
  invariant(registry.schema === "pipelinenews.v8.fast-registry.v1", "registry schema mismatch");
  ...
  invariant(projectPayload.generation === GENERATION, "project generation mismatch");
  invariant(projectPayload.cache_identity === EXPECTED_CACHE_IDENTITY, "project cache identity mismatch");
  ...
  apply({ syncUrl: false });
  document.getElementById("releaseMeta").textContent = `V8 fast candidate · ${rows.length.toLocaleString("en-GB")} canonical projects · ${registry.performance.maximum_physical_project_rows} physical rows · full details loaded only on demand`;
...
boot().catch((error) => {
  console.error(error);
  document.getElementById("tbody").innerHTML = '<tr><td colspan="11" class="fast-fail">Canonical Q2 REPD data unavailable. The fast candidate has failed closed.</td></tr>';
