...
async function boot() {
  ...
  apply({ syncUrl: false });
  document.getElementById("releaseMeta").textContent = `Live News Discovery + chronology candidate · ${rows.length.toLocaleString("en-GB")} canonical projects · ${registry.performance.maximum_physical_project_rows} physical rows · full details loaded only on demand · NOT DEPLOYED`;
