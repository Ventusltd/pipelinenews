...
const GENERATION = "202609010836";
// The immutable data cartridges were sealed under the original
// generation and are referenced, never copied; their names do not
// change when the release is recompiled.
const DATA_GENERATION = "202608261927";
...
const PINNED_SHA256 = Object.freeze({
  ...
  "ui/javascript/202608261804-projects-v9-5-1.js": "af631896921c418943014362b21a967310adb4e85c2cc8e6e99582358ca6194e",
...
function rewriteModule(sourcePath, source) {
  if (sourcePath.endsWith("202608261632-state.js")) {
    source = replaceExactly(source, "../../dist/major_project_news_v5.json", `../data/news/${DATA_GENERATION}-major-project-news-v9-5-1.json`);
    source = replaceExactly(source, "https://raw.githubusercontent.com/Ventusltd/globalgrid2050/main/dist/major_project_news_v5.json", `../data/news/${DATA_GENERATION}-major-project-news-v9-5-1.json`);
  }
  if (sourcePath.endsWith("202608261752-canonical-projects-v9-1.js")) {
    source = replaceExactly(source, "contracts/release.v9.1.json", `../data/contracts/${DATA_GENERATION}-release-v9-1.json`);
...
async function compileManifest(inputRecords) {
  ...
  for (let index = 0; index < manifest.project_partitions.length; index += 1) {
    ...
    const cartridge = `data/projects/${DATA_GENERATION}-project-partition-v9-1-${String(index + 1).padStart(2, "0")}.json`;
  ...
  for (const entry of manifest.atlas_partitions) {
    ...
    const cartridge = `data/atlas/${DATA_GENERATION}-atlas-${technology}-partition-v9-1-${match[2]}.geojson`;
    const bytes = await readFile(absolute(cartridge));
