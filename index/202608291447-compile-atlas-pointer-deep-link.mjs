...
const SOURCE_PARENT = "fabd905977410c10be3008e2ab5ceacccfce7930";
...
const COMPILER_METHOD = "pipelinenews-atlas-pointer-exact-identity-successor-v2";
const IDENTITY_ROUTING = Object.freeze({
...
const SOURCE = Object.freeze({
  manifest: `manifests/${GENERATION}-atlas-pointer-deep-link-successor.json`,
  cartridge: `ui/cartridges/${GENERATION}-atlas-pointer-deep-link.mjs`,
...
const OUTPUT = Object.freeze({
  ...
  atlasCartridge: `releases/${RELEASE_ID}/assets/${GENERATION}-atlas-pointer-deep-link.mjs`,
...
function parseArgs(argv) {
  ...
  for (const key of ["parent-root", "out-root", "source-commit", "source-committed-at", "atlas-receiver"]) {
...
function compileHtml(parentBytes) {
  ...
  html = replaceExactly(html, "<title>GlobalGrid2050 | PipelineNews seven-topic sector intelligence candidate</title>", "<title>PipelineNews | Atlas V9 deep-link successor 202608291447</title>");
...
function compileRegistry(parent, sourceCommit, compilerRaw, cartridgeRaw, dependencies, receiver) {
  ...
  registry.cartridges.atlas_v9_deep_link = {
    ...
    path: `assets/${GENERATION}-atlas-pointer-deep-link.mjs`,
    ...
    receiver_pointer_commit: receiver.resolved_commit,
  ...
  registry.cache_contract = {
    ...
    compiler: {
      path: `index/${GENERATION}-compile-atlas-pointer-deep-link.mjs`,
    ...
    atlas_deep_link: {
      ...
      target: {
        base_url: ATLAS_BASE,
        pathname: receiver.receiver.route,
      },
      receiver_pointer: receiver.pointer,
      receiver_pointer_commit: receiver.resolved_commit,
      contractual_golden_repd_ref: receiver.receiver.golden_repd_ref,
      fallback: receiver.fallback,
      identity_anchor: "repd_ref",
      query_parameter_order: ["repd_ref"],
      ...IDENTITY_ROUTING,
...
function compileRuntime(parentBytes, cacheIdentity) {
  ...
  runtime = replaceExactly(runtime, 'import { buildAtlasV8DeepLink } from "./202608271329-atlas-v8-deep-link-cartridge.js";', `import { buildAtlasV9DeepLink } from "./${GENERATION}-atlas-pointer-deep-link.mjs";`);
  ...
  runtime = replaceExactly(runtime, 'let query = "";', 'let query = "";\nlet requestedRepdRef = "";');
  ...
  runtime = replaceExactly(runtime, '  query = parameters.get("q") || "";', '  const repdRefParameter = parameters.get("repd_ref") || "";\n  requestedRepdRef = /^\\d+$/u.test(repdRefParameter) ? repdRefParameter : "";\n  query = requestedRepdRef || (parameters.get("q") || "");');
  runtime = replaceExactly(runtime, '    if (county !== "All" && item.county !== county) continue;', '    if (county !== "All" && item.county !== county) continue;\n    if (requestedRepdRef && String(item.repd_ref) !== requestedRepdRef) continue;');
  runtime = replaceExactly(runtime, '  if (query) url.searchParams.set("q", query);', '  if (requestedRepdRef) url.searchParams.set("repd_ref", requestedRepdRef);\n  else if (query) url.searchParams.set("q", query);');
  runtime = replaceExactly(runtime, '  query = "";\n  sortMode = "capacity_desc";', '  query = "";\n  requestedRepdRef = "";\n  sortMode = "capacity_desc";');
  runtime = replaceExactly(runtime, '    query = event.target.value.trim();', '    requestedRepdRef = "";\n    query = event.target.value.trim();');
...
async function main() {
  ...
  const receiverRaw = await readFile(path.resolve(args["atlas-receiver"]));
  const receiver = JSON.parse(receiverRaw);
  assert.equal(receiver.schema, "pipelinenews.gridatlas-pointer-receipt.v2");
  assert.equal(receiver.classification, "VERIFIED_GRIDATLAS_LIVE_POINTER");
  assert.equal(receiver.repository, "Ventusltd/gridatlas");
  assert.equal(receiver.receiver.query_parameter, "repd_ref");
  assert.equal(receiver.receiver.identity_rule, "EXACT_REPD_REF_ONLY");
  assert.equal(receiver.authentication?.receiver_ui_parity, "NOT_ASSERTED");
  assert.match(receiver.receiver.golden_repd_ref, /^\d+$/u);
  assert.match(receiver.resolved_commit, /^[0-9a-f]{40}$/u);
  assert.match(receiver.pointer.sha256, /^[0-9a-f]{64}$/u);
  const resolvedBase = new URL(receiver.receiver.base_url);
  assert.equal(resolvedBase.protocol, "https:");
  assert.equal(resolvedBase.hostname, "ventusltd.github.io");
  assert.equal(resolvedBase.pathname, receiver.receiver.route);
  assert.equal(resolvedBase.search, "");
  assert.equal(resolvedBase.hash, "");
  ATLAS_BASE = resolvedBase.href;
  const sourceManifest = JSON.parse(await readFile(safePath(ROOT, SOURCE.manifest)));
  assert.equal(sourceManifest.schema, "pipelinenews.atlas-pointer-deep-link-successor-source.v2");
  ...
  assert.equal(sourceManifest.gridatlas_pointer.repository, receiver.repository);
  assert.equal(sourceManifest.gridatlas_pointer.path, receiver.pointer.path);
  assert.equal(sourceManifest.gridatlas_pointer.expected_commit, receiver.resolved_commit);
  assert.equal(sourceManifest.gridatlas_pointer.expected_bytes, receiver.pointer.bytes);
  assert.equal(sourceManifest.gridatlas_pointer.expected_sha256, receiver.pointer.sha256);
  assert.deepEqual(sourceManifest.gridatlas_pointer.immutable_release_manifest, receiver.release_manifest);
  assert.equal(sourceManifest.gridatlas_pointer.query_parameter, receiver.receiver.query_parameter);
  assert.equal(sourceManifest.gridatlas_pointer.identity_rule, receiver.receiver.identity_rule);
  assert.equal(sourceManifest.gridatlas_pointer.contractual_golden_repd_ref, receiver.receiver.golden_repd_ref);
  assert.equal(sourceManifest.gridatlas_pointer.hardcoded_route_forbidden, true);
  assert.equal(sourceManifest.atlas_v9_receiver.resolution, "AUTHENTICATED_FROM_GRIDATLAS_CURRENT_V3");
  assert.equal(sourceManifest.atlas_v9_receiver.generation, receiver.receiver.generation);
  assert.equal(sourceManifest.atlas_v9_receiver.release_id, receiver.receiver.release_id);
  assert.equal(sourceManifest.atlas_v9_receiver.source_commit, receiver.receiver.source_commit);
  assert.equal(sourceManifest.atlas_v9_receiver.publication_commit, receiver.receiver.publication_commit);
  assert.equal(sourceManifest.atlas_v9_receiver.identity_rule, receiver.receiver.identity_rule);
  assert.equal(sourceManifest.atlas_v9_receiver.ui_parity, receiver.authentication.receiver_ui_parity);
  assert.equal(Object.hasOwn(sourceManifest.atlas_v9_receiver, "base_url"), false, "source manifest must not hardcode the receiver route");
  assert.deepEqual(sourceManifest.routing_contract.identity_matching, IDENTITY_ROUTING);
  ...
  const cartridgeTemplateRaw = await readFile(safePath(ROOT, SOURCE.cartridge));
  const cartridgeRaw = Buffer.from(replaceExactly(
    cartridgeTemplateRaw.toString("utf8"),
    "__GRIDATLAS_RECEIVER_JSON__",
  ...
  const registry = compileRegistry(parentRegistry, sourceCommit, compilerRaw, cartridgeRaw, dependencies, receiver);
  ...
  const buildManifest = {
    ...
    provenance_files: 1,
    gridatlas_receiver_evidence: {
      receipt_sha256: sha256(receiverRaw),
      pointer: receiver.pointer,
      mirror: receiver.mirror,
      release_manifest: receiver.release_manifest,
      resolved_commit: receiver.resolved_commit,
  ...
  const releaseManifest = {
    ...
    generation: GENERATION,
    release_id: RELEASE_ID,
    incepted_at: "2026-08-29T14:47:00Z",
    source_commit: sourceCommit,
    source_parent_commit: SOURCE_PARENT,
    ...
    atlas_v9_deep_link: {
      base_url: ATLAS_BASE,
      pointer: receiver.pointer,
      pointer_commit: receiver.resolved_commit,
      release_manifest: receiver.release_manifest,
      query_parameter_order: ["repd_ref"],
      inbound_compatibility_alias: "repd_ref",
      ...IDENTITY_ROUTING,
      golden_repd_ref: receiver.receiver.golden_repd_ref,
      golden_url: `${ATLAS_BASE}?repd_ref=${receiver.receiver.golden_repd_ref}`,
      selection_evidence: ["receiver URL", "selected source", "receiver card"],
      map_links: 7652,
      no_map: 28,
      synthetic_receiver_allowed: false,
      optional_sentinels: ["17494", "13599"],
      fallback: receiver.fallback,
    },
    folder_contract: {
    ...
    publication_control: {
      ...
      source_workflow_stage_branch: `ci/${GENERATION}-atlas-pointer-release`,
  ...
  process.stdout.write(`${JSON.stringify({
    classification: "COMPILED_ATLAS_POINTER_FOLDER_SUCCESSOR",
    generation: GENERATION,
    release_id: RELEASE_ID,
    files: files.size,
    ...
    receiver_pointer_sha256: receiver.pointer.sha256,
    receiver_release_id: receiver.receiver.release_id,
  }, null, 2)}\n`);
}
