...
export const GENERATION = "202608272130";
...
const APPROVED_SOURCE_CLOSURE = Object.freeze([
  ...
  { id: "GOVUK_HORMUZ_ENERGY", topic_code: "ENERGY_SECURITY_HORMUZ", adapter: "GOVUK_SEARCH_V1", source_url: "https://www.gov.uk/api/search.json?count=6&order=-public_timestamp&q=Strait%20of%20Hormuz%20energy", item_hosts: ["www.gov.uk"] },
  { id: "GOVUK_UKRAINE_ENERGY", topic_code: "ENERGY_SECURITY_UKRAINE", adapter: "GOVUK_SEARCH_V1", source_url: "https://www.gov.uk/api/search.json?count=6&order=-public_timestamp&q=Ukraine%20energy%20security", item_hosts: ["www.gov.uk"] },
...
export function validateContract(contract) {
  ...
  assert.equal(contract.topics.length, 7);
  const topicRanks = new Map(contract.topics.map(({ code, display_rank }) => [code, display_rank]));
  assert.equal(topicRanks.size, 7);
  assert.deepEqual([...topicRanks.values()], [1, 2, 3, 4, 5, 6, 7]);
