...
const admittedTechnologies = new Set(["Solar Photovoltaics", "Battery", "Wind Onshore", "Wind Offshore"]);
...
const sourceRows = parseCsv(csvBytes.toString("utf8"));
if (sourceRows.length < 1000 || !Object.hasOwn(sourceRows[0] || {}, "Ref ID")) throw new Error("REPD source failed schema/row-count validation");
const scopedRows = sourceRows.filter((row) => admittedTechnologies.has(row["Technology Type"]) && Number(row["Installed Capacity (MWelec)"]) >= 1);
const ids = scopedRows.map((row) => String(row["Ref ID"]).trim());
if (ids.some((id) => !id) || new Set(ids).size !== ids.length) throw new Error("scoped REPD source contains blank or duplicate Ref IDs");
...
const previous = await optionalJson(statePath, { source_sha256: null, scoped_row_hashes: {} });
const rowHashes = Object.fromEntries(scopedRows.map((row) => [String(row["Ref ID"]).trim(), sha256(JSON.stringify(row))]));
const sourceHash = sha256(csvBytes);
const newRows = scopedRows.filter((row) => !admittedIds.has(String(row["Ref ID"]).trim()));
...
const priorHashes = previous.scoped_row_hashes || {};
const changedIds = previous.source_sha256 && previous.source_sha256 !== sourceHash ? ids.filter((id) => priorHashes[id] && priorHashes[id] !== rowHashes[id]) : [];
const checkedAt = new Date().toISOString();

await atomicJson(candidatePath, {
  schema: "pipelinenews.repd-source-candidate.v2",
  ...
  source: { title: attachment.title, url: attachment.url, govuk_updated_at: metadata.public_updated_at, sha256: sourceHash, bytes: csvBytes.length, raw_rows: sourceRows.length },
  scope: { technologies: [...admittedTechnologies], capacity_mw_inclusive_minimum: 1, scoped_rows: scopedRows.length, all_official_statuses: true },
  ...
  new_projects: newRows.map(publicProject),
  admission: "CANDIDATE_ONLY_REQUIRES_FAIL_CLOSED_QUARTERLY_RELEASE",
});
await atomicJson(statePath, { schema: "pipelinenews.repd-source-state.v2", checked_at: checkedAt, source_url: attachment.url, source_sha256: sourceHash, raw_rows: sourceRows.length, scoped_rows: scopedRows.length, scoped_row_hashes: rowHashes });
console.log(`REPD ${attachment.title}: ${sourceRows.length} raw; ${scopedRows.length} in scope; ${newRows.length} new; ${changedIds.length} changed; ${missingIds.length} missing`);
